...
import com.dtt.logs.service.impl.*;
...
@RequestMapping("/authn-log/api")
@RestController
public class AuthnLogController {

    private final AuthnLogImpl auditService;

    private final AuthnLogPageImpl pageService;


    MongoTemplate mongoTemplate;
    public AuthnLogController(AuthnLogImpl auditService,AuthnLogPageImpl pageService,MongoTemplate mongoTemplate) {
    ...
    @KafkaListener(topics = "${auth.log.topic}", groupId = "kyc-logs", containerFactory = "auditDTOServiceContainerFactory")
    public void getMessage(AuditDTO auditDTO) {
        try {
            System.out.println("Listening to auth.log.topic");
    ...
    @GetMapping("get/authentication/count")
    public com.dtt.logs.dto.ApiResponse getAuthenticationTypeCount(@RequestParam String identifier) {
    return auditService.getAuthenticationTypeCount(identifier);
    }
