...
@RequestMapping("/ob-log/api")
@RestController
public class ObLogController {

    private final ObLogImpl auditService;

    private final ObLogPageImpl pageService;


    MongoTemplate mongoTemplate;

    public ObLogController(ObLogImpl auditService, ObLogPageImpl pageService,@Qualifier("obLogMongoTemplate") MongoTemplate mongoTemplate) {
        this.auditService = auditService;
        this.pageService = pageService;
        this.mongoTemplate = mongoTemplate;
    }

    @KafkaListener(topics = "${ob.log.topic}", groupId = "kyc-logs", containerFactory = "auditDTOServiceContainerFactory")
    public void getMessage(AuditDTO auditDTO) {
        System.out.println("Listening to ob.log.topic : Dto : "+auditDTO);
        try {
    ...
    public ApiResponse onboardingLog(@RequestBody OtpDTO otp) {
        ...
        try {
            ...
            List<ServiceAuditLog> auditLogs = mongoTemplate.find(query.with(Sort.by(Sort.Direction.DESC, "timestamp")),
                    ServiceAuditLog.class, "auditlogs");
            System.out.println("Count of logs with suid "+ auditLogs.size());
            List<ServiceAuditLog> auditLog1 = mongoTemplate.find(query1.with(Sort.by(Sort.Direction.DESC, "timestamp")),
                    ServiceAuditLog.class, "auditlogs");
            System.out.println("Count of logs with encrypt email "+ auditLog1.size());
            for (ServiceAuditLog auditLog : auditLogs) {
    ...
    private String encryptedString(String s) {
        try {
            ...
            System.out.println("Encrypted value "+ result.getResponse());
